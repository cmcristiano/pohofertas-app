...
const App = () => {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      setCurrentSlide(p =>
        p === carouselProducts.length - 1 ? 0 : p + 1
      );
    }, 4500);
        ...
        <div className="max-w-7xl mx-auto px-4 py-3">
          <div className="flex justify-between mb-3">
            ...
            <div className="flex gap-2">
              <a href="https://ofertas.pohofertas.com.br" target="_blank"
                className="bg-black text-white px-3 py-2 rounded-full text-xs font-bold">
                🔥 VER OFERTAS
              </a>
              <a href="https://wa.me/5511999999999" target="_blank"
                className="bg-green-600 text-white px-3 py-2 rounded-full text-xs font-bold flex gap-1">
        ...
        <section className="max-w-7xl mx-auto px-4 mt-4 mb-6">
          <div className="relative overflow-hidden rounded-2xl shadow-lg">
            {carouselProducts.map((p, index) => (
              <div
                key={p.id}
                className={`${index === currentSlide ? 'opacity-100' : 'opacity-0 absolute inset-0'} 
                  bg-gradient-to-r from-orange-500 to-orange-600 transition-all`}
              >
                <div className="grid md:grid-cols-2 gap-6 p-6 text-white">
                  <div>
                    ...
                    <a href={p.link} target="_blank"
                      className="inline-flex mt-4 bg-white text-black px-5 py-3 rounded-full font-bold">
